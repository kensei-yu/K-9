        ...
        citations.forEach(citation => {
            ...
            citationLink.addEventListener('click', (e) => {
                ...
                handlePreview(citation.path).catch(error => {
                    ...
                    appendMessage('bot', `プレビューエラー: ${error.message}`);
                }).finally(() => {
...
// /ingest
async function handleIngest(paths) {
    ...
    if (!response.ok)
        throw new Error('バックエンドからの応答が不正です (取り込み)。');
    const data = await response.json();
...
async function handleStats() {
    ...
    if (!response.ok)
        throw new Error('バックエンドからの応答が不正です (統計)。');
    const data = await response.json();
